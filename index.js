...
const createGame = (container) => {
  ...
  const checkTeleport = ([X, Y], mapSize) => {
    return [(mapSize + X) % mapSize, (mapSize + Y) % mapSize]
  }
  ...
  const filterIllegalPositions = (neighborPositions) =>
    neighborPositions.filter(([X, Y]) => _isInRange(X) && _isInRange(Y))

  const filterUnavailableFields = (positions) =>
    positions.filter((position) => isAvailableField(WOLF, position))
  ...
  const getDistancesFromRabbit = (positions) => {
    ...
    const distances = positions.map((position) =>
      calculateDistanceFromRabbit(position)
    )
    return {
      distances,
      positions,
  ...
  const getShortestDistancePosition = ({ distances, positions }) =>
  ...
  const wolvesStep = () => {
    getCharacterPositions(WOLF).forEach((wolf) => {
  ...
  const rabbitStep = ([x, y]) => {
    const rabbit = getCharacterPositions(RABBIT)[0]
    const nextPosition = checkTeleport(
